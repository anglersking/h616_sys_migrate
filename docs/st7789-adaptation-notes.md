...
画面或画面位置不对。实板 ANSI 红/蓝色块测试确认该模块使用驱动的原生 RGB
顺序，因此不能添加可选的 `bgr;` 属性；添加后会让红色和蓝色互换。白色终端文字
无法暴露这个问题，必须使用彩色字符或图片验证。
...
```bash
printf '\033[31mRED 红色\033[0m  \033[34mBLUE 蓝色\033[0m\n'
```

本项目验证模块不需要 `bgr;`。如果换了另一批面板且上述测试确实红蓝互换，再为
对应硬件单独加入该属性，不要只凭白色登录界面判断颜色顺序。
